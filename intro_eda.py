...
def feature_cleaner(df, low, high):
    ...
    """
    quantile_low = df.quantile(q=low, axis=0)
    quantile_high = df.quantile(q=high, axis=0)
    flags = (df > quantile_low) & (df < quantile_high)
    df2 = df[flags].dropna()
    df2 = (df2 - df2.mean(axis=0))/df2.std(axis=0)
    return df2


def calc_r(df, chosen_class):
    df0 = df[df['CLASS'] == chosen_class].drop('CLASS', axis=1)
    min_0 = df0.min(axis=0)
    max_0 = df0.max(axis=0)
    var_0 = df0.var(axis=0)
...
def one_hot_encode(label_to_encode, labels):
    ...
    """
    return [1 if labels[i] == label_to_encode else 0 for i in range(len(labels))]
