...
def diff_in_days(df):
    ...
    """
    df['difference_days'] = abs(df['time_2']-df['time_1'])/(60*60*24)
    # df['difference_days'] = df['difference_days'].apply(lambda x: trunc(x))
    df['difference_days'] = df['difference_days'].apply(trunc)
    return df[['difference_days']]
...
def return_location(df):
    ...
    """
    df['short_name'] = df['locations'].apply(lambda x: json.loads(x)['short_name'])
...
def return_post_codes(df):
    ...
    """
    df['postcodes'] = df['text'].str.findall(r'[A-Z]{1,2}[0-9][0-9A-Z]?\s*[0-9][A-Z]{2}').apply(' | '.join)
    return df[['postcodes']]
